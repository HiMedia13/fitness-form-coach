...
| 단위 | 시점 | 내용 |
|------|------|------|
| **렙** | 렙 완료 즉시 | 그 렙의 점수·교정 큐 (실시간 피드백) |
| **유지** | 플랭크 등 유지 종료 | 유지 시간·자세 평가 |
| **세트** | `s` 키 또는 **휴식 자동 감지**(약 12초 무동작) | 세트 전체 집계 코칭 — 총 렙 수, 깊이 일관성(min/max), 평균 템포, **반복된 자세 이슈 빈도**, 렙별 깊이 추세로 본 피로 징후, 다음 세트 핵심 큐 |
